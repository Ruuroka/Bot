...
async function startNazeBot() {
	...
	naze.ev.on('call', async (call) => {
		...
		if (db.set[botNumber].anticall) {
			for (let id of call) {
				if (id.status === 'offer') {
					let msg = await naze.sendMessage(id.from, { text: `Jangan nelpon dong, ${id.isVideo ? 'Video' : 'Suara'} aku lagi jelek nih.\nkalo @${id.from.split('@')[0]} butuh Bantuan, langsung chat ownernya aja :D`, mentions: [id.from]});
